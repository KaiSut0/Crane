...
namespace Crane.Patterns
{
    public class TwistOrigamize
    {
        ...
        public TwistOrigamize(Mesh mesh, double shrinkRatio, double offsetRatio, double trancateRatio, double rotation)
        {
            ...
            for (int i = 0; i < mesh.Vertices.Count; i++)
            {
                ...
                foreach (var tEdge in tEdges)
                {
                    ...
                    if (faces.Length == 2)
                    {
                        ...
                        MountainLines.Add(new Line(star.Vertices[top], star.Vertices[rFrom]));

                        if(edgeCount == 0)
                        {
                            star.Faces.AddFace(top, top + tEdges.Length - 1, rFrom);
                            MountainLines.Add(new Line(star.Vertices[top + tEdges.Length - 1], star.Vertices[rFrom]));
                        }
                        else
                        {
                            star.Faces.AddFace(top, top - 1, rFrom);
                            MountainLines.Add(new Line(star.Vertices[top - 1], star.Vertices[rFrom]));
                        }
